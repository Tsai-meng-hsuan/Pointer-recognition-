...
def identify_gauge(picture,min_R):
    img = picture
    ...
    #霍夫圓檢測
    circles = cv2.HoughCircles(dst_img,cv2.HOUGH_GRADIENT,1,500,param1=70,param2=30,minRadius=min_R,maxRadius=0)
...
while True:
    ...
    aa = aa+1
    try:
        img = identify_gauge(frame,200)[1]
        angel_list.append(identify_gauge(frame,150)[0])
        cv2.imshow("img",img)
        cv2.waitKey(1)
    except:
        print("error: ")
